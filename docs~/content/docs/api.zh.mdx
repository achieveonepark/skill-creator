...
{
    void Load(IDataLoader<T> loader);   // 全量替换
    void Add(IDataLoader<T> loader);    // 合并
    void Add(T item);
    T Get(string id);
...
// ConditionRegistry、TriggerRegistry 也采用相同的模式
```
...
public sealed class StatSheet
{
    ...
    float Get(string statKey);   // 应用修正器后的最终值
